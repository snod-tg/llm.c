...
void softmax_forward(float* probs, float* logits, int B, int T, int V, int Vp) {
    ...
    for (int b = 0; b < B; b++) {
        for (int t = 0; t < T; t++) {
            // probs <- softmax(logits) 计算坐标
            float* logits_bt = logits + b * T * Vp + t * Vp;
            float* probs_bt = probs + b * T * Vp + t * Vp;

            // maxval is only calculated and subtracted for numerical stability 获取 最大值
            float maxval = -10000.0f; // TODO something better
            for (int i = 0; i < V; i++) {
            ...
            // note we only loop to V, leaving the padded dimensions 计算每一个位置的概率
            for (int i = 0; i < V; i++) {
                probs_bt[i] /= sum;
            }
            // for extra super safety we may wish to include this too,
            // forcing the probabilities here to be zero, but it shouldn't matter 填充
            for (int i = V; i < Vp; i++) {
                probs_bt[i] = 0.0f;
            }
        }
